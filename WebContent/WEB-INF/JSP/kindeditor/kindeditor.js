...
KE.setting = {
	...
	items : [
		...
		'superscript', '|', 'selectall', 'preview','-',
		'title', 'fontname', 'fontsize', '|', 'textcolor', 'bgcolor', 'bold',
		'italic', 'underline', 'strikethrough', 'removeformat', '|', 'image','copy', 
		'flash', 'media', 'advtable', 'hr','link', 'unlink','template'
	],
	colorTable : [
...
KE.lang = {
	...
	invalidDoc:'請選擇有效的文檔類型。\n只允許pdf,doc,docx,xls,xlsx,ppt,pptx格式。',
	invalidDocTitle:'檔案說明不能為空.',
...
KE.plugin['copy'] = {
	getSelectedNode : function(id) {
		...
		if (startNode.tagName.toLowerCase() != 'copy') return;
	...
	check : function(id) {
		...
		var title = KE.$('docTitle', dialogDoc).value;
		var urlBox;
		urlBox = KE.$('docFile', dialogDoc);

		if (!urlBox.value.match(/\.(pdf|doc|xls|ppt|docx|xlsx|pptx)(\s|\?|$)/i)) {
			alert(KE.lang['invalidDoc']);
			urlBox.focus();
			return false;
		}
		if(title==""){
			alert(KE.lang['invalidDocTitle']);
			urlBox.focus();
			return false;
		}
